...
describe('Bucketlist reducer', () => {
  ...
  it('should load bucketlists when passed LOAD_ALL_BUCKETLISTS_SUCCESS', () => {
    const newBucketlists = { bucketlists: [{ id: 3, name: 'ngoitsi' }], nextUrl: '', previousUrl: '' };
    const action = actions.loadAllBucketlistsSuccess(newBucketlists);
    const newState = bucketlistReducer(initialState, action);
  ...
  it('should add item when passed LOAD_ALL_BUCKETLISTS_SUCCESS', () => {
